/* Shop, Cart, and Checkout Logic (Firebase Edition) */
...
console.log("Shop Cart Module Evaluated Immediately");

// --- Global Global Wrappers for OnClick (Nuclear Option for Events) ---
window.addToCartClick = function (btn) {
    if (!btn) return;
    console.log("Direct OnClick: Add to Cart", btn);
    ...
    // Visual Feedback
    if (span) span.textContent = 'Added! ✔️';
    ...
    setTimeout(() => {
        ...
        btn.setAttribute('data-hover', originalText);
        btn.classList.remove('is-added-state');
    }, 2000);
};

window.openDetailsClick = function (btn) {
    if (!btn) return;
    openProductModal({
        id: btn.getAttribute('data-id'),
        title: btn.getAttribute('data-title'),
...
// (Required because this is now a module)
...
function openProductModal(product) {
    ...
    newBtn.addEventListener('click', () => {
        ...
        // Keeping it open as per original code preference, just creating a small delay if needed.
        setTimeout(() => {
            // Optional: Close logic if desired
        }, 500);
    });

    // Show Modal (Bootstrap 5 vanilla JS way or jQuery)
    if (typeof $ !== 'undefined') {
            ...
            row.innerHTML = `
                ...
                <td>
                    <div class="quantity-input">
                        <button class="quantity-down" onclick="window.updateQuantity('${item.id}', -1)">--</button>
                        <input class="quantity" type="text" value="${item.quantity}" readonly>
                        <button class="quantity-up" onclick="window.updateQuantity('${item.id}', 1)">+</button>
                    </div>
                </td>
...
function updateCheckoutSummary() {
    const summaryContainer = document.querySelector('.checkout-summary-tbody');
...
function setupCheckoutPage() {
    ...
    placeOrderBtn.addEventListener('click', function (e) {
        ...
        addDoc(collection(db, ORDERS_COLLECTION), orderData)
            .then(docRef => {
                console.log("Order written with ID: ", docRef.id);

                // --- GOOGLE SHEET SUBMISSION (Dual Write) ---
...
function submitToGoogleSheet(data) {
    const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbw4YvkpUDud3FclE0_qV1LgIefohke3PI2OKjUVSP_mw8mnHZSzuwsK_fyuhADlOmUW/exec';

    // Flatten data for the sheet
    const payload = new FormData();
    payload.append('action', 'createOrder');
    payload.append('orderId', data.id || 'N/A'); // Firestore ID might not be ready if async, but that's fine
    payload.append('date', data.orderDate);
    payload.append('customerName', `${data['first-name']} ${data['last-name']}`);
    payload.append('email', data.email);
    payload.append('phone', data.number);
    payload.append('address', `${data['street-name']}, ${data.city}, ${data.state}, ${data.zip}, ${data.country}`);
    payload.append('total', data.totalAmount);
    payload.append('paymentMethod', data.paymentMethod);

    // Format items string
    const itemsStr = data.items.map(i => `${i.title} (x${i.quantity})`).join(', ');
    payload.append('items', itemsStr);

    // Send using no-cors to avoid blocking
    fetch(SCRIPT_URL, {
        method: 'POST',
        mode: 'no-cors',
        body: payload
...
function initShopCart() {
    console.log("Shop Cart Logic Initializing...");
    ...
    if (couponForm) {
        couponForm.addEventListener('submit', async function (e) {
            ...
            try {
                ...
                const q = query(collection(db, COUPONS_COLLECTION), where("code", "==", code), where("status", "==", "active"));
...
function setupShopPage() {
    ...
    document.addEventListener('click', function (e) {
        // console.log("Click detected on:", e.target); // Optional: Comment out to reduce noise if confirmed working
