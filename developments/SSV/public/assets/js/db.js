...
const DBService = {
    ...
    createClub: async (clubData) => {
        try {
            // clubData: { name, description, staffIds: [], isActive: true, createdAt }
    ...
    getClubs: async (onlyActive = false) => {
        try {
    ...
    updateClub: async (id, data) => {
        try {
            await db.collection('clubs').doc(id).update(data);
            return { success: true };
        } catch (error) {
            return { success: false, message: error.message };
        }
    ...
    // Soft delete usually means setting isActive to false, but if we want 'delete' logic separately
    // Note: The requirement says "Activate / Deactivate Clubs (soft delete preferred)".
    // So updateClub with { isActive: false } is sufficient, but let's provide a helper if needed.
    toggleClubStatus: async (id, isActive) => {
        return DBService.updateClub(id, { isActive });
    }
};
