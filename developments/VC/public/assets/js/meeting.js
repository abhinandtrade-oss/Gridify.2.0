...
let isSuperAdmin = false;      // NEW: Tracks strict Super Admin (Email based)
let isAdminRole = false;       // NEW: Tracks Admin Role (DB based)
...
// Helper: Get Admin Status
// Helper: Get Admin Status
async function getAdminStatus(user) {
    if (!user) return { isSuper: false, isAdmin: false };

    let status = { isSuper: false, isAdmin: false };
    ...
    try {
        ...
        if (userDoc.exists()) {
            ...
            if (role === 'super_admin') {
                status.isSuper = true;
                status.isAdmin = true; // Super admin implies admin
            } else if (role === 'admin') {
                status.isAdmin = true; // Admins get moderator privileges (Kick, Mute, etc.)
            }
            // NOTE: Regular 'admin' role users retain their badge (handled in checkVerification)
            // but do NOT get host privileges in meetings.
        }
    } catch (e) {
...
async function joinMeeting() {
    try {
        ...
        const adminStatus = await getAdminStatus(currentUser);
        isSuperAdmin = adminStatus.isSuper;
        isAdminRole = adminStatus.isAdmin;
        hasHostPrivileges = (hostUid === currentUser.uid) || isSuperAdmin || isAdminRole;
...
function showGuestEntryUI() {
    ...
    const handleJoin = async () => {
        ...
        try {
            ...
            // Create Guest User Doc in Firestore
...
async function initializeMeetingMediaAndLogic() {
    ...
    // Only the main Super Admin account gets the HO identity (Database Driven)
    if (isSuperAdmin) {
    ...
    if (localLabel) {
        if (displayName === "Gridify Tec Team HO") {
            ...
            // Check verification for self (including 'admin' shield)
...
function initChat() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach((doc) => {
            ...
            div.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                    <div class="chat-sender" style="${senderStyle}">
                        <span id="chat-sender-${doc.id}">${data.sender}</span>
                        ${data.sender === "Gridify Tec Team HO" ? '<i class="fas fa-check-circle" style="margin-left: 4px; color: #ef4444;"></i>' : ''} 
                        ${isMsgFromHost ? '<span style="font-size:0.7em; border: 1px solid var(--success); padding: 0 4px; border-radius: 4px; margin-left: 4px;">HOST</span>' : ''}
                    </div>
                    ${deleteBtn}
                </div>
            ...
            // Check for Admin Badge (if not HO)
            if (data.sender !== "Gridify Tec Team HO") {
                checkVerification(data.uid, `chat-sender-${doc.id}`);
            }
        });
...
function setupHostControls() {
    // RESTRICTION: Regular Admins can moderate (kick/mute) but CANNOT End the meeting for everyone.
    // Only the actual Host or a Super Admin can end the meeting.
    if (!isSuperAdmin && currentUser.uid !== hostUid) return;
...
async function checkVerification(uid, elementId) {
    if (verifiedCache.has(uid)) {
        appendBadge(elementId, verifiedCache.get(uid));
        return;
    }
    try {
        ...
        if (snap.exists()) {
            ...
            if (data.role === 'admin') {
                verifiedCache.set(uid, 'admin');
                appendBadge(elementId, 'admin');
            } else if (data.isVerified) {
                verifiedCache.set(uid, 'verified');
                appendBadge(elementId, 'verified');
            }
        }
    } catch (e) { }
}

function appendBadge(elementId, type) {
    const el = document.getElementById(elementId);
    if (!el) return;
    // Check if parent (or el) already has the icon
    if (el.parentNode.querySelector('.badge-icon')) return;

    const icon = document.createElement('i');
    icon.classList.add('badge-icon');

    if (type === 'admin') {
        icon.className = 'fas fa-shield-alt badge-icon';
        icon.style.cssText = 'margin-left: 6px; color: #8b5cf6; font-size: 0.9em;'; // Purple for Admin
        icon.title = "Admin";
    } else {
        icon.className = 'fas fa-check-circle badge-icon';
        icon.style.cssText = 'margin-left: 4px; color: #3b82f6; font-size: 0.9em;'; // Blue for Verified
        icon.title = "Verified";
    }

    // Append to parent of the name span
...
function startHeartbeat() {
    ...
    const pulse = async () => {
        try {
            // 1. Always Update User Activity (User is on the page -> User is Active)
            ...
            // 2. Update Meeting Activity (Only if active)
            const meetingRef = doc(db, "meetings", meetingId);
    ...
    pulse(); // Run immediately
    heartbeatInterval = setInterval(pulse, 30000);
}
