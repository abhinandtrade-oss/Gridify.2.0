document.addEventListener('DOMContentLoaded', () => {
    ...
    const setupDynamicSection = (containerId, buttonId, templateId, renderCallback) => {
        ...
        const addItem = () => {
            ...
            // AI Button listeners for dynamic items
            newItem.querySelectorAll('.ai-enhance-exp').forEach(btn => {
                btn.addEventListener('click', handleAIEnhanceExperience);
            });
            newItem.querySelectorAll('.ai-enhance-proj').forEach(btn => {
                btn.addEventListener('click', handleAIEnhanceProject);
            });
        ...
        try {
            ...
            const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
                ...
                headers: {
                    ...
                    "HTTP-Referer": "https://gridify.in",
                    "X-Title": "Gridify Resume Builder",
                    "Content-Type": "application/json"
                },
                body: JSON.stringify({
                    "model": model,
                    "messages": [
                        { "role": "system", "content": "You are a professional resume writer and career coach. Your task is to enhance resume content to be ATS-friendly, professional, and impactful. Use active verbs and quantify achievements where possible. Keep it concise." },
            ...
            if (response.status === 429) {
                if (!useFallback) {
                    console.log("Primary model rate limited, trying fallback...");
                    return callAI(prompt, true);
                }
                alert("AI Rate Limit: Both primary and fallback models are busy. Please wait a few seconds.");
                return null;
            }
            ...
            if (data.choices && data.choices[0] && data.choices[0].message) {
                ...
                if (data.error && data.error.message) {
                    alert(`AI Status: ${data.error.message}`);
                }
            ...
            if (!useFallback) return callAI(prompt, true);
            return null;
        }
    };

    const handleAIEnhanceSummary = async () => {
        const btn = document.getElementById('aiEnhanceSummary');
        const summaryTxt = document.getElementById('summary');
        const jd = document.getElementById('jobDescription').value;
        const jobTitle = document.getElementById('jobTitle').value;

        if (!summaryTxt.value && !jd) {
            alert("Please enter some summary text or a Job Description first.");
            return;
        }
        ...
        const prompt = `
            Task: Enhance this professional summary for a resume.
            User Title: ${jobTitle}
            Current Summary: ${summaryTxt.value}
            Target Job Description: ${jd}

            Instructions: 
            - Make it compelling and professional.
            - Focus on key skills and achievements.
            - Keep it to 3-4 impactful lines.
            - If a Job Description is provided, tailor the summary to match its requirements.
            - Return ONLY the enhanced summary text. No preamble.
        `;

        const result = await callAI(prompt);
        if (result) {
            summaryTxt.value = result;
            summaryTxt.dispatchEvent(new Event('input'));
        } else {
    ...
    const handleAIEnhanceExperience = async (e) => {
        const btn = e.target;
        const item = btn.closest('.dynamic-item');
        const descArea = item.querySelector('.exp-desc');
        const title = item.querySelector('.exp-title').value;
        const company = item.querySelector('.exp-company').value;
        const jd = document.getElementById('jobDescription').value;

        if (!descArea.value && !jd) {
            alert("Please provide some responsibilities or a Job Description.");
            return;
        }
        ...
        const prompt = `
            Task: Enhance these professional responsibilities for a resume.
            Title: ${title} at ${company}
            Current Responsibilities: ${descArea.value}
            Target Job Description: ${jd}
            ...
            - Tailor to the provided Job Description if available.
            - Return ONLY the enhanced bullet points. No preamble.
        `;

        const result = await callAI(prompt);
        if (result) {
            descArea.value = result;
    ...
    const handleAIEnhanceProject = async (e) => {
        const btn = e.target;
        const item = btn.closest('.dynamic-item');
        const descArea = item.querySelector('.proj-desc');
        const name = item.querySelector('.proj-name').value;
        const jd = document.getElementById('jobDescription').value;

        if (!descArea.value && !jd) {
            alert("Please provide a project description or a Job Description.");
            return;
        }
        ...
        const prompt = `
            Task: Enhance this project description for a resume.
            Project Name: ${name}
            Current Description: ${descArea.value}
            Target Job Description: ${jd}

            Instructions:
            - Focus on technical impact and problem solving.
            - Use bullet points (starting with "- ").
            - Tailor to the target Job Description if available.
    ...
    const handleAISuggestSkills = async () => {
        const btn = document.getElementById('aiSuggestSkills');
        const jd = document.getElementById('jobDescription').value;
        const jobTitle = document.getElementById('jobTitle').value;

        if (!jobTitle && !jd) {
            alert("Please enter a Job Title or Job Description to get skill suggestions.");
            return;
        }
        ...
        const prompt = `
            Task: Suggest top 10-15 relevant skills for a resume based on the following:
            ...
            - Return ONLY the comma-separated list. No preamble or numbering.
            - Example output: React, Node.js, Project Management, Agile, Communication
        `;

        const result = await callAI(prompt);
        if (result) {
            const skills = result.split(',').map(s => s.trim()).filter(s => s);
            const container = document.getElementById('skillsContainer');
            ...
            skills.forEach(skill => {
                ...
                container.appendChild(clone);
                const newItem = container.lastElementChild;
                newItem.querySelector('.skill-name').value = skill;

                // Add event listeners as in setupDynamicSection
                newItem.querySelectorAll('input, textarea, select').forEach(inp => {
                    inp.addEventListener('input', renderSkills);
                });
                newItem.querySelector('.remove-btn').addEventListener('click', () => {
                    newItem.remove();
                    renderSkills();
    ...
    document.getElementById('aiEnhanceSummary').addEventListener('click', handleAIEnhanceSummary);
    document.getElementById('aiSuggestSkills').addEventListener('click', handleAISuggestSkills);
