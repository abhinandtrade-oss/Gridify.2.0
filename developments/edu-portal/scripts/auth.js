...
const Auth = {
    ...
    showToast: (message, type = 'info') => {
        ...
        // --- NEW: Persist to Notification Center ---
        // Only if user is logged in
        if (Auth.currentUser && window.DB && typeof window.DB.addNotification === 'function') {
            // We avoid "Loading..." messages or generic "Error" placeholders if possible, but basic catch-all is fine per request.
            // Check for avoid-loops (if DB.addNotification calls showToast on error)
            // Ideally DB.addNotification should NOT call showToast to avoid infinite recursion.
            // Based on previous view of DB.addNotification, it only console.errors.

            // Map Toast type to Notification type (essentially same strings)

            // We execute this asynchronously so it doesn't block the UI
            setTimeout(() => {
                window.DB.addNotification(Auth.currentUser.uid, {
        ...
        try {
            ...
            if (!userDoc.exists) {
                ...
                userData = {
                    ...
                    authMethod: 'google',
                    admissionNumber: await (async () => {
                        try {
                            // 1. Get Site Name
                            const settingsDoc = await window.db.collection('settings').doc('general').get();
                            const siteName = (settingsDoc.exists && settingsDoc.data().siteName) ? settingsDoc.data().siteName : 'EduPortal';
                            const namePrefix = siteName.substring(0, 2).toUpperCase();

                            // 2. Get Date (YYYYMMDD)
                            const now = new Date();
                            const dateStr = now.toISOString().split('T')[0].replace(/-/g, '');

                            // 3. Get User Count (This determines uniqueness roughly)
                            const usersSnap = await window.db.collection('users').get();
                            const count = usersSnap.size + 1; // +1 for current user

                            return `${namePrefix}-${dateStr}-${count}`;
                        } catch (e) {
                            console.error("Error generating admission number:", e);
                            return `ST-${new Date().toISOString().split('T')[0].replace(/-/g, '')}-0`;
                        }
                    })()
                };
    ...
    createStaffUser: async (email, password, name, role) => {
        ...
        try {
            ...
            await window.db.collection('users').doc(user.uid).set({
                ...
                admissionNumber: await (async () => {
                    if (role !== 'student') return null; // Only for students
                    try {
                        // 1. Get Site Name
                        const settingsDoc = await window.db.collection('settings').doc('general').get();
                        const siteName = (settingsDoc.exists && settingsDoc.data().siteName) ? settingsDoc.data().siteName : 'EduPortal';
