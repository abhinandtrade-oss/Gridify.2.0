...
const DB = {
    ...
    async updateMobileNumber(uid, mobile) {
        try {
            ...
            const existingUser = await window.db.collection('users')
                .where('mobile', '==', mobile)
                .get();
            ...
            await window.db.collection('users').doc(uid).update({ mobile: mobile });
            return { success: true };
        } catch (e) {
            console.error("Error updating mobile number:", e);
    ...
    async searchUsers(query) {
        try {
            ...
            const emailSnapshot = await usersRef.where('email', '>=', query).where('email', '<=', query + '\uf8ff').limit(10).get();
            if (!emailSnapshot.empty) {
                return emailSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            }

            // Try searching by name (case-sensitive unfortunately in generic Firestore without indexing)
            // For better search, we usually need a third-party service like Algolia or a specific lowercase field.
            // We will just try exact match or prefix for now.
            const nameSnapshot = await usersRef.where('name', '>=', query).where('name', '<=', query + '\uf8ff').limit(10).get();
            if (!nameSnapshot.empty) {
                return nameSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            }

            // Try Mobile
            const mobileSnapshot = await usersRef.where('mobile', '==', query).limit(1).get();
            if (!mobileSnapshot.empty) {
                return mobileSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            }
    ...
    // --- Legal Updates Approval System ---
    async submitLegalPageUpdate(data) {
        // data: { type, content, userId, userName, role }
        try {
            await window.db.collection('pending_legal_updates').add({
                ...data,
                status: 'pending',
                createdAt: new Date()
            });
            ...
            console.error("Error submitting legal update:", e);
            throw e;
        }
    },

    async getPendingLegalUpdates() {
        try {
            const snapshot = await window.db.collection('pending_legal_updates')
                .where('status', '==', 'pending')
                .get();
            const updates = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
            // Sort in memory by date desc (newest first)
            return updates.sort((a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0));
        } catch (e) {
            console.error("Error getting pending legal updates:", e);
            return [];
        }
    },

    async approveLegalUpdate(updateId) {
        try {
            const docRef = window.db.collection('pending_legal_updates').doc(updateId);
            const doc = await docRef.get();
            if (!doc.exists) throw new Error("Update request not found");
            const data = doc.data();

            // 1. Save content to actual location
            if (data.type === 'privacy' && this.savePrivacyPolicy) {
                await this.savePrivacyPolicy(data.content);
            } else if (data.type === 'terms' && this.saveTermsOfService) {
                await this.saveTermsOfService(data.content);
            }
            ...
            if (this.addNotification) {
                await this.addNotification(data.userId, {
                    title: 'Legal Page Update Approved',
                    message: `Your update for ${data.type === 'privacy' ? 'Privacy Policy' : 'Terms of Service'} has been approved and published.`,
                    type: 'success',
            ...
            console.error("Error approving legal update:", e);
            throw e;
        }
    },

    async rejectLegalUpdate(updateId, reason) {
        try {
            const docRef = window.db.collection('pending_legal_updates').doc(updateId);
            const doc = await docRef.get();
            if (!doc.exists) throw new Error("Update request not found");
            ...
            if (this.addNotification) {
                await this.addNotification(data.userId, {
                    title: 'Legal Page Update Rejected',
                    message: `Your update for ${data.type === 'privacy' ? 'Privacy Policy' : 'Terms of Service'} was rejected.\nReason: ${reason}`,
                    type: 'error',
                    link: 'admin/index.html'
                });
            }
            ...
            console.error("Error rejecting legal update:", e);
            throw e;
        }
    },

    async getLegalUpdatesByUser(uid) {
        try {
            const snapshot = await window.db.collection('pending_legal_updates')
                .where('userId', '==', uid)
                .get();
            ...
            console.error("Error getting user legal updates:", e);
            return [];
        }
    },

    async checkOverdueLegalUpdates() {
        try {
            // Check for pending updates older than 24 hours
            const now = new Date();
            ...
            snapshot.docs.forEach(doc => {
                ...
                if (createdAt < cutoff) {
                    ...
                    console.log(`[EMAIL SIMULATION] To: admin@gridify.in | Subject: Overdue Legal Page Update Review | Body: Update ID ${doc.id} is pending for more than 24 hours.`);
                }
            });
            return overdueCount;
        } catch (e) {
            console.error("Error checking overdue updates:", e);
            return 0;
        }
    }
};
